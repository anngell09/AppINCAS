...
while True:
    ...
    if boxes is not None:
        for box in boxes:
            ...
            # Descargar los descriptores faciales del modelo
            descriptors_webcam = embedding_webcam.detach().numpy()

            # Comparar el rostro con la imagen de referencia
            similarity = F.cosine_similarity(torch.from_numpy(descriptors_webcam), torch.from_numpy(referencia_embedding)).item()
